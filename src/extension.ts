...
// Finds the project root directory by looking for 'build.zig'
function findProjectRoot(directory: string): string | null {
  const buildZigPath = path.join(directory, "build.zig");
  if (fs.existsSync(buildZigPath)) {
    return directory;
  }
...
// Gets the binary name from the project root directory
function getBinaryNameFromRoot(filePath: string): string | null {
...
export function activate(context: vscode.ExtensionContext) {
  diagnosticCollection =
    vscode.languages.createDiagnosticCollection("Zig extras");
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "zig-language-extras.runSingleTest",
      runSingleTest
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "zig-language-extras.runFileTests",
      runFileTests
    )
  );
  context.subscriptions.push(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "zig-language-extras.buildWorkspace",
      buildWorkspace
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "zig-language-extras.testWorkspace",
      testWorkspace
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "zig-language-extras.debugBinary",
      debugBinary
    )
  );
}
...
function getDebugEnv() {
  ...
  return {
    testBinaryPath:
      config.get<string>("testBinaryPath") || defaultTestBinaryPath,
  };
}
...
function getEnv(findCurrentTest: boolean = true) {
  ...
  const binName =
    getBinaryNameFromRoot(fileName) || path.parse(fileNameRelative).name;
...
function getDebugType(): string {
  const config = vscode.workspace.getConfiguration("zig-language-extras");
  return (
    config.get<string>("debugType") || getDebugTypeForPlatform(os.platform())
  );
}
